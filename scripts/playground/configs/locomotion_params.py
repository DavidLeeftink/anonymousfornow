...
def rsl_rl_config(
  ...
  rl_config = config_dict.create(
      ...
      # logging
      save_interval=16_000,  # check for potential saves every this many iterations
  ...
  if env_name in (
    ...
    rl_config.max_iterations = 10_000
  if env_name == "Go1JoystickFlatTerrain":
    rl_config.algorithm.learning_rate = 3e-4 
    # rl_config.num_steps_per_env=24*4,
    rl_config.algorithm.schedule = "fixed"
  if env_name == "H1JoystickGaitTracking" or env_name =="G1JoystickFlatTerrain":
    rl_config.max_iterations = 25_000
