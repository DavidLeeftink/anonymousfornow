...
def get_rl_config(env_name: str) -> config_dict.ConfigDict:
  ...
  if env_name in registry.manipulation._envs:
    ...
    from dmc_partial_obs import print_obs_layout
    print_obs_layout("WalkerWalk")
...
def adapt_playground_config(
    ...
    # --- Recurrent-only extensions ------------------------------------------
    if not use_rnn:
        if costate_coeff:
            raise ValueError(
                f"--costate_coeff={costate_coeff} requires --use_rnn=True; an "
                f"MLP policy has no hidden state to regress."
            )
        if readout != "linear":
            raise ValueError(
                f"--readout={readout} requires --use_rnn=True; MLPModel uses "
                f"its own head."
            )
    old_algorithm["costate_coeff"] = costate_coeff if use_rnn else 0.0
    ...
    if use_rnn:
        ...
        # Readout head. Recurrent only: for MLPModel the head is the model.
        cfg["actor"]["readout"] = readout
        cfg["critic"]["readout"] = "mlp"
        cfg["actor"]["squash_output"] = True

    return cfg

def build_env_overrides(env_cfg) -> dict:
  """Turns the env-knob flags into config_overrides, failing loudly.

  A silently-ignored --history_len would produce a run directory labelled
  hist8 that actually trained with hist1, which is worse than a crash because
  it looks like data.
  """
  overrides = {}
...
def main(argv):
  ...
  policy.reset()
  eval_env = registry.load(_ENV_NAME.value, config=env_cfg, config_overrides=env_cfg_overrides)
  ...
  try:
      ...
      frames = eval_env.render(traj, camera=None, height=480*2, width=640*2, scene_option=scene_option)
